...
workflow MAG {
    ...
    */
    ...
    //     }
        ...
        ch_input_for_postbinning_bins        = Channel.fromPath(params.refined_bins)
                                                .map { bin ->
                                                        def meta = [:]
                                                        meta.group        = 0
                                                        return [ meta, bin ]
        ...
        ch_input_for_postbinning_unbins      = Channel.fromPath(params.refined_unbins)
                                                .map { bin ->
                                                        def meta = [:]
                                                        meta.group        = 0
                                                        return [ meta, bin ]
        ...
        ch_input_for_postbinning_bins_unbins = ch_input_for_postbinning_bins.mix(ch_input_for_postbinning_unbins)
        ch_input_for_binsummary              = Channel.value(file(params.refined_depths_summary))
